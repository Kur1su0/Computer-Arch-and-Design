if { ![project_exists {mem_delay}] } {
     project_new {mem_delay}
} else {
     project_open {mem_delay}
}
set_project_settings -cmp {mem_delay}
if { ![project_settings_exist -sim mem_delay] } {
        set_project_settings -sim mem_delay
}
set_global_assignment -section_id {mem_delay} -name {EDA_DESIGN_ENTRY_SYNTHESIS_TOOL} {PRECISION SYNTHESIS}
set_global_assignment -name {EDIF_FILE} {mem_delay.edf}
...
if { [file exists {mem_delay_altera.prop}] } {
  source mem_delay_altera.prop
}
set_global_assignment -name {FAMILY} {MAXV}
set_global_assignment -section_id {mem_delay} -name {DEVICE} {5M2210ZF324A5}
set_global_assignment -name {PROJECT_SHOW_ENTITY_NAME} {OFF}
set_global_assignment -name {SDC_FILE} {mem_delay_pnr_constraints.sdc}
