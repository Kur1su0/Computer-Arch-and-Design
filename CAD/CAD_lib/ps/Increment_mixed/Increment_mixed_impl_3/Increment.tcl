if { ![project_exists {Increment}] } {
     project_new {Increment}
} else {
     project_open {Increment}
}
set_project_settings -cmp {Increment}
if { ![project_settings_exist -sim Increment] } {
        set_project_settings -sim Increment
}
set_global_assignment -section_id {Increment} -name {EDA_DESIGN_ENTRY_SYNTHESIS_TOOL} {PRECISION SYNTHESIS}
set_global_assignment -name {EDIF_FILE} {Increment.edf}
...
if { [file exists {Increment_altera.prop}] } {
  source Increment_altera.prop
}
set_global_assignment -name {FAMILY} {MAXV}
set_global_assignment -section_id {Increment} -name {DEVICE} {5M2210ZF324A5}
set_global_assignment -name {PROJECT_SHOW_ENTITY_NAME} {OFF}
set_global_assignment -name {SDC_FILE} {Increment_pnr_constraints.sdc}
