if { ![project_exists {mem}] } {
     project_new {mem}
} else {
     project_open {mem}
}
set_project_settings -cmp {mem}
if { ![project_settings_exist -sim mem] } {
        set_project_settings -sim mem
}
set_global_assignment -section_id {mem} -name {EDA_DESIGN_ENTRY_SYNTHESIS_TOOL} {PRECISION SYNTHESIS}
set_global_assignment -name {EDIF_FILE} {mem.edf}
...
if { [file exists {mem_altera.prop}] } {
  source mem_altera.prop
}
set_global_assignment -name {FAMILY} {MAXV}
set_global_assignment -section_id {mem} -name {DEVICE} {5M2210ZF324A5}
set_global_assignment -name {PROJECT_SHOW_ENTITY_NAME} {OFF}
set_global_assignment -name {SDC_FILE} {mem_pnr_constraints.sdc}
