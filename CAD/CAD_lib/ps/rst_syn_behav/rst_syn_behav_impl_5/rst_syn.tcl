if { ![project_exists {rst_syn}] } {
     project_new {rst_syn}
} else {
     project_open {rst_syn}
}
set_project_settings -cmp {rst_syn}
if { ![project_settings_exist -sim rst_syn] } {
        set_project_settings -sim rst_syn
}
set_global_assignment -section_id {rst_syn} -name {EDA_DESIGN_ENTRY_SYNTHESIS_TOOL} {PRECISION SYNTHESIS}
set_global_assignment -name {EDIF_FILE} {rst_syn.edf}
...
if { [file exists {rst_syn_altera.prop}] } {
  source rst_syn_altera.prop
}
set_global_assignment -name {FAMILY} {MAXV}
set_global_assignment -section_id {rst_syn} -name {DEVICE} {5M2210ZF324A5}
set_global_assignment -name {PROJECT_SHOW_ENTITY_NAME} {OFF}
set_global_assignment -name {SDC_FILE} {rst_syn_pnr_constraints.sdc}
