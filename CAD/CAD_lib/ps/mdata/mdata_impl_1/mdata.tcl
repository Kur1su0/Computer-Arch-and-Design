if { ![project_exists {mdata}] } {
     project_new {mdata}
} else {
     project_open {mdata}
}
set_project_settings -cmp {mdata}
if { ![project_settings_exist -sim mdata] } {
        set_project_settings -sim mdata
}
set_global_assignment -section_id {mdata} -name {EDA_DESIGN_ENTRY_SYNTHESIS_TOOL} {PRECISION SYNTHESIS}
set_global_assignment -name {EDIF_FILE} {mdata.edf}
...
if { [file exists {mdata_altera.prop}] } {
  source mdata_altera.prop
}
set_global_assignment -name {FAMILY} {MAXV}
set_global_assignment -section_id {mdata} -name {DEVICE} {5M2210ZF324A5}
set_global_assignment -name {PROJECT_SHOW_ENTITY_NAME} {OFF}
set_global_assignment -name {SDC_FILE} {mdata_pnr_constraints.sdc}
