if { ![project_exists {not_gate}] } {
     project_new {not_gate}
} else {
     project_open {not_gate}
}
set_project_settings -cmp {not_gate}
if { ![project_settings_exist -sim not_gate] } {
        set_project_settings -sim not_gate
}
set_global_assignment -section_id {not_gate} -name {EDA_DESIGN_ENTRY_SYNTHESIS_TOOL} {PRECISION SYNTHESIS}
set_global_assignment -name {EDIF_FILE} {not_gate.edf}
...
if { [file exists {not_gate_altera.prop}] } {
  source not_gate_altera.prop
}
set_global_assignment -name {FAMILY} {MAXV}
set_global_assignment -section_id {not_gate} -name {DEVICE} {5M2210ZF324A5}
set_global_assignment -name {PROJECT_SHOW_ENTITY_NAME} {OFF}
set_global_assignment -name {SDC_FILE} {not_gate_pnr_constraints.sdc}
