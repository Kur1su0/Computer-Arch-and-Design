if { ![project_exists {nop_ctrl}] } {
     project_new {nop_ctrl}
} else {
     project_open {nop_ctrl}
}
set_project_settings -cmp {nop_ctrl}
if { ![project_settings_exist -sim nop_ctrl] } {
        set_project_settings -sim nop_ctrl
}
set_global_assignment -section_id {nop_ctrl} -name {EDA_DESIGN_ENTRY_SYNTHESIS_TOOL} {PRECISION SYNTHESIS}
set_global_assignment -name {EDIF_FILE} {nop_ctrl.edf}
...
if { [file exists {nop_ctrl_altera.prop}] } {
  source nop_ctrl_altera.prop
}
set_global_assignment -name {FAMILY} {MAXV}
set_global_assignment -section_id {nop_ctrl} -name {DEVICE} {5M2210ZF324A5}
set_global_assignment -name {PROJECT_SHOW_ENTITY_NAME} {OFF}
set_global_assignment -name {SDC_FILE} {nop_ctrl_pnr_constraints.sdc}
